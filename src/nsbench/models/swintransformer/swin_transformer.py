...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, padding_mode: str = "constant"):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
    ...
    def forward(self, x, mask_matrix):
        ...
        x = F.pad(x, (0, 0, pad_l, pad_r, pad_t, pad_b), mode=self.padding_mode)
        _, Hp, Wp, _ = x.shape
...
class PatchMerging(nn.Module):
    ...
    Args:
        ...
        norm_layer (nn.Module, optional): Normalization layer.  Default: nn.LayerNorm
    """
    def __init__(self, dim, norm_layer=nn.LayerNorm, padding_mode: str = "constant"):
        super().__init__()
        self.dim = dim
        self.padding_mode = padding_mode
...
class BasicLayer(nn.Module):
    ...
    def __init__(self,
        ...
        if downsample is not None:
            self.downsample = downsample(dim=dim, norm_layer=norm_layer, padding_mode=padding_mode)
        else:
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None, padding_mode: str = "constant"):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if W % self.patch_size[1] != 0:
            x = F.pad(x, (0, self.patch_size[1] - W % self.patch_size[1]), mode=self.padding_mode)
        if H % self.patch_size[0] != 0:
            x = F.pad(x, (0, 0, 0, self.patch_size[0] - H % self.patch_size[0]), mode=self.padding_mode)
...
class SwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        in_chans = in_chans*context_size
        ...
        resolution = pretrain_img_size//patch_size
    ...
    def forward(self, x: torch.Tensor, teacher_forcing_steps: int=50) -> torch.Tensor:
        """
        ...
        """
        # x: [B, T, D, H, W]
        ...
        for t in range(x.shape[1]):
            ...
            if t < teacher_forcing_steps:  # Teacher forcing: Feed observations into the model
                ...
                if self.context_size == 0:
                    ...
                    x_t = torch.cat([x_obs, x_out], dim=1)
            ...
            if t < self.context_size - 1:
                ...
                out = x_t[:, -1] + self.one_step(einops.rearrange(x_t, "b t d h w -> b (t d) h w"))
