...
class AFNONet(nn.Module):
    def __init__(
        ...
        self.in_chans = in_chans*context_size
    ...
    def forward(self, x, teacher_forcing_steps: int=50):
        outs = []
        ...
        for t in range(x.shape[1]):
            ...
            if t < teacher_forcing_steps:  # Teacher forcing: Feed observations into the model
                x_t_in = x[:, max(0, t-(self.context_size-1)):t+1]
                ...
                if self.context_size == 0:
                    ...
                    x_t_in = torch.cat([x_obs, x_out], dim=1)
            ...
            if t < self.context_size - 1:
                ...
                x_t = self.forward_features(rearrange(x_t_in, "b t d h w -> b (t d) h w"))
                x_t = self.head(x_t)
                x_t = rearrange(
                    x_t,
